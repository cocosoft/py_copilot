...
def get_all_models(db: Session = Depends(get_db)):
    ...
    for model in models:
        ...
        model_response = ModelWithSupplierResponse(
            id=str(model.id),
            name=str(getattr(model, "model_name", model.model_id)) if getattr(model, "model_name", model.model_id) is not None else "Unknown Model",
            display_name=str(getattr(model, "model_name", model.model_id)) if getattr(model, "model_name", model.model_id) is not None else "Unknown Model",
...
# 注意：test-api 路由已移至 app.api.v1.supplier_model，以避免重复注册
# 该模块中的 test-api 路由已被删除
