...
| model_id | String | 供应商提供的模型ID | 否 |
| name | String | 模型名称 | 是 |
| display_name | String | 显示名称 | 是 |
| description | Text | 模型描述 | 否 |
| type | String | 模型类型（chat/completion/embedding） | 否 |
| context_window | Integer | 上下文窗口大小 | 否 |
| max_tokens | Integer | 最大生成长度 | 否 |
| default_temperature | Float | 默认温度参数 | 否 |
| default_max_tokens | Integer | 默认最大生成长度 | 否 |
| default_top_p | Float | 默认top_p参数 | 否 |
| default_frequency_penalty | Float | 默认频率惩罚 | 否 |
| default_presence_penalty | Float | 默认存在惩罚 | 否 |
| custom_params | JSON | 自定义参数 | 否 |
| is_active | Boolean | 是否启用 | 是 |
| is_default | Boolean | 是否为默认模型 | 是 |
| created_at | DateTime | 创建时间 | 否 |
| updated_at | DateTime | 更新时间 | 否 |
...
| name | String | 能力名称（如text_generation） | 是 |
| display_name | String | 显示名称（如文本生成） | 是 |
